...
def clean_text(text: Any) -> str:
    if text is None:
        return ""
    s = str(text)
    # Keep it conservative: only trim whitespace
    return s.strip()
...
def to_messages(turns: List[Dict[str, Any]]) -> Tuple[List[Dict[str, Any]], List[Dict[str, Any]]]:
    """
    Returns:
      - messages_for_app: [{senderType, content}]
      - messages_for_finetune: [{role, content}]
    """
    ...
    for t in turns:
        ...
        if sender is None or role is None:
            # Unknown speaker label: skip rather than corrupting the dataset
            continue
...
def main() -> None:
    ...
    # Single finetune, no tone split / no rewrite. For polite+casual + ASCII paths, use:
    #   node scripts/convert_counsel_jsonl.mjs
