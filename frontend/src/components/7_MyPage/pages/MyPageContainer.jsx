  ...
  const fetchLetters = async () => {
    ...
    try {
      ...
      const transformed = {
        ...
        featured: lr.items?.[0] ? {
          id: lr.items[0].id,
          unread: lr.items[0].unread,
          title: lr.items[0].title,
          sender: lr.items[0].sender || '메이티',
          preview: lr.items[0].preview,
          date: lr.items[0].date,
          status: lr.items[0].unread ? `새 편지 ${lr.unreadCount}` : '읽음',
        } : {
          id: null,
          unread: false,
          title: '도착한 편지가 없어요',
