...
public class EmotionReportService {
    ...
    private final EmotionReportDAO reportDAO;
    private final ObjectMapper objectMapper; // Jackson 라이브러리를 활용한 JSON 파싱

    public EmotionReportDTO getLatestReport(long userId) {
        EmotionReportDTO report = reportDAO.selectLatestReportByUserId(userId);
        if (report != null) {
    ...
    public EmotionReportDTO getReportDetail(long reportId) {
        EmotionReportDTO report = reportDAO.selectReportById(reportId);
        if (report != null) {
            parseJsonFields(report);
        }
        return report;
    ...
    public EmotionReportDTO generateNewReport(long userId) {
    ...
    private void parseJsonFields(EmotionReportDTO dto) {
        try {
