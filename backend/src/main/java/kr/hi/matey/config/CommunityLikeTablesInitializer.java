 ...
 * POST dislike_count와 맞추기 위한 사용자별 싫어요 기록 테이블.
    ...
    public void afterPropertiesSet() {
        ...
        String postDislike = """
                CREATE TABLE IF NOT EXISTS `user_post_dislike` (
                  ...
                  `created_at` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
                  PRIMARY KEY (`user_id`, `post_id`),
                  KEY `idx_upd_post` (`post_id`),
                  CONSTRAINT `fk_upd_user` FOREIGN KEY (`user_id`) REFERENCES `USER` (`user_id`) ON DELETE CASCADE,
                  CONSTRAINT `fk_upd_post` FOREIGN KEY (`post_id`) REFERENCES `POST` (`post_id`) ON DELETE CASCADE
                ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4
                """;
        try (Connection conn = dataSource.getConnection(); Statement st = conn.createStatement()) {
            st.execute(postLike);
            st.execute(commentLike);
            st.execute(postDislike);
        } catch (Exception e) {
